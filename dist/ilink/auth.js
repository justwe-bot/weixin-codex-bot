...
export async function startQrLogin(baseUrl = DEFAULT_ILINK_BASE_URL) {
...
export async function checkQrLogin(baseUrl, qrcode) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), QR_POLL_TIMEOUT_MS);
    try {
        const response = await fetch(buildUrl(baseUrl, `ilink/bot/get_qrcode_status?qrcode=${encodeURIComponent(qrcode)}`), {
            headers: {
...
export function toLoginResult(status, fallbackBaseUrl) {

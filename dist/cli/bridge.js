...
import { clearCodexSession, getCodexSession, getContextToken, loadBridgeConfig, loadCredentials, loadCursor, saveCursor, setCodexSession, setContextToken, } from "../state.js";
    ...
    while (true) {
        try {
            ...
            if ((response.ret && response.ret !== 0) || (response.errcode && response.errcode !== 0)) {
                ...
                throw new Error(`getupdates returned ret=${response.ret ?? 0}, errcode=${response.errcode ?? 0}, errmsg=${response.errmsg ?? ""}`);
            }
            for (const message of response.msgs ?? []) {
                try {
                    ...
                    if (fromUserId && contextToken) {
                        ...
                        await client.sendText(fromUserId, `处理你的消息时出错：${reason}`, contextToken).catch(() => { });
                    }

...
import type { BridgeConfig } from "../state.js";
...
function buildPrompt(input: CodexRunInput): string {
  return [
...
function maybePush(args: string[], flag: string, value?: string): void {
  if (value) {
    args.push(flag, value);
  }
}

function extractSessionId(candidate: unknown): string | undefined {
...
export async function runCodexPrompt(input: CodexRunInput): Promise<CodexRunResult> {
  ...
  return new Promise<CodexRunResult>((resolve, reject) => {
    ...
    let stdout = "";
    let stderr = "";
    let discoveredSessionId: string | undefined = input.sessionId;
